...
public class Gravite {
	...
	public Gravite(Dekor d){
		decor = d;
		gravite();
	}
	public Gravite(Dekor d,JPanel j){
		decor = d;
		creation=j;
		gravite();
	}
	
	public static boolean estPose(Cible c,Dekor d){
	...
	public static boolean estStable(Dekor d){
	...
	public static int estStable(Cible c,Dekor d) {
		for(Cible c2 : d.getCibles()){
			if(estPoseSur(c, c2)){
